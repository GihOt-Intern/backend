...
public class GameMapGridDeserializer extends JsonDeserializer<GameMapGrid> {
    ...
    public GameMapGrid deserialize(JsonParser parser, DeserializationContext ctxt) throws IOException {
        ...
        Coordinate cornerA = new Coordinate(
                cornerANode.get("x").asDouble(),
                cornerANode.get("z").asDouble()
        );

        JsonNode cornerBNode = node.get("cornerB");
        Coordinate cornerB = new Coordinate(
                cornerBNode.get("x").asDouble(),
                cornerBNode.get("z").asDouble()
        );
        ...
        return new GameMapGrid(id, name, cornerA, cornerB, nRows, nCols, grid);
    }
}
